...
<%
	...
	String query = "select count(*) from kc_reportlist where category = '" + category + "' " + where; 
	String[] totcount = new DBHandler().getOneColumnData("kc", query);
	int totpage = (Integer.parseInt(totcount[0]) + (row_cnt - 1)) / row_cnt;

	query = "select a.* from (select rownum rn, a.* from (select id, category, title, url, to_char(idate, 'yyyymmddhh24miss') datetime" +
			" from kc_reportlist where category = '" + category + "' " + where +  " order by " + orderby + ")a)a  WHERE a.rn >= " + start + " and a.rn <= " + end;
	...
	for (String[] row : data) {
		...
		try {
			cal.setTime(new java.text.SimpleDateFormat("yyyyMMddHHmmss").parse(row[5]));
